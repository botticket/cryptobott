                    ...
                    if float(Close) > float(CloseY):
                        if float(Close) >= float(CloseM) :
                            if float(CloseY) >= float(ema):
                                notice = 'Buy CloseY >> {}'.format(high_trend)
                                start = 'oM {}'.format(OpenM)
                                buy = 'cM {} | cY{}'.format(CloseM,CloseY)
                                stop = 'e {} {}%'.format(ema,pema)
                                target = 'H {} | L {}'.format(max_Y,min_Y)
                                avg = '$ {} \nr{} | {}'.format(comvluee,m_RSI,pattern)
                            elif float(CloseM) >= float(ema):
                                if float(candle_end) >= float(candle_start):
                                    notice = 'Buy CloseM >> {}'.format(high_trend)
                                    start = 'oM {}'.format(OpenM)
                                    buy = 'cY{} | cM {}'.format(CloseY,CloseM)
                                    stop = 'e {} {}%'.format(ema,pema)
                                    target = 'H {} | L {}'.format(max_Y,min_Y)
                                    avg = '$ {} \nr{} | {}'.format(comvluee,m_RSI,pattern)
                                else:
